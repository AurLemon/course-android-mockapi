<template>
  <div class="bg-[var(--color-surface-0)] h-full flex-row">
    <div class="w-48 bg-[var(--color-surface-2)] p-4 flex flex-col">
      <div class="user-info flex mt-auto pt-4">
        <div class="name mb-2">
          <div class="nick text-2xl font-medium">{{ authStore.user?.username }}</div>
      ...
      <button
        @click="logout"
        class="logout text-white p-1 w-full bg-red-500 hover:bg-red-600 rounded-md transition-colors"
      >
        退出登录
      </button>
    </div>
